...
{
	var self = this;
	this.text = this.Text = function (message, options)
	{
		return $('<div style="white-space: pre-wrap;"></div>').text(message).modalDialog(options);
	}
	this.html = this.Html = function (message, options)
	{
		return $('<div></div>').html(message).modalDialog(options);
	}
	...
	var Confirm = function (questionEle, onYes, onNo, options)
	{
		...
		var dlg = $dlg.modalDialog(options);
		$yes.click(function ()
		{
			dlg.close();
